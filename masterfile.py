        ...
        if selected_item:
            ...
            if values:
                ...
                def edit_remarks():
                    ...
                    edit_window.title("Edit Diagnosis")
                    ...
                    date_entry = CTkEntry(edit_window, font=("Arial", 14))
                    date_entry.insert(0, str(current_date))
                    date_entry.pack(pady=5)

                    # Diagnosis field
                    diagnosis_label = CTkLabel(edit_window, text="Diagnosis:", font=("Arial", 14))
                    diagnosis_label.pack(pady=5)
                    diagnosis_entry = CTkEntry(edit_window, font=("Arial", 14), width=300)
                    diagnosis_entry.insert(0, current_diag)
                    diagnosis_entry.pack(pady=5)

                    def save_changes():
                        new_date = date_entry.get()
                        new_diagnosis = diagnosis_entry.get()
                    ...
                    save_button = CTkButton(edit_window, text="Save Changes", command=save_changes)
                ...
                diagnosis_table.heading("diag_id", text="ID")  # You can hide this column if you want
                ...
                edit_details_Button = CTkButton(details_window, text='Edit Diagnosis',command=edit_remarks, width=210, height=60,
...
def fetch_events_for_selected_date():
    ...
    # Normalize the date format to match your MySQL database (YYYY-MM-DD)
    try:
...
treeviewframe.place(x=320, y=150, width=800, height=600)
scrollbarX = Scrollbar(treeviewframe, orient=HORIZONTAL)
scrollbarY = Scrollbar(treeviewframe, orient=VERTICAL)
...
inventoryrecord=CTkButton(window,text='Stock Ordering',state=DISABLED,width=250,height=45,corner_radius=0,font=("arial",14,'bold'),border_width=2,border_color='#1A5319',fg_color="#387478",hover_color='#729762')
...
#------------------------------------------------------------------------------------------------------------------
searchentry = CTkEntry(window,placeholder_text="Search Owner's Name",width=200)
...
searchLabel = CTkLabel(window, text="( Surname,   First Name   M.I )",font=('arial',11,'italic'),bg_color='white')
searchLabel.place (x=265,y=88)
#------------------------------------------------------------------------------------------------------------------
...
# Calendar widget
cal = Calendar(window, selectmode='day', date_pattern="yyyy-mm-dd",font=("Arial", 33,'italic'),showweeknumbers=False,showothermonthdays=False,background = '#90EE90',selectbackground = '#FFB6C1')
cal.place(x=1150,y=150)
...
event_frame = CTkFrame (window, width=570,height=220, border_color='#6A9C89', border_width=4, fg_color='#C7DBB8')
event_frame.place(x=912,y=467)
event_label = CTkLabel(event_frame, text="Instruction:",font=('arial',16,'italic','bold'))
event_label.place(x=10,y=10)
event_description1 = CTkLabel(event_frame,text="  To add a new appointment, follow the",font=('bold',12,'italic'))
event_description1.place(x=18,y=30)
event_description2 = CTkLabel(event_frame,text="example below ;",font=('bold',12,'italic'))
event_description2.place(x=10,y=50)
event_description2 = CTkLabel(event_frame,text="Pet's Name (Species) - Reason of Appointment ",font=('arial',10,'italic','bold'))
...
# Add event button
add_event_btn = CTkButton(window, text="Add", bg_color='#C7DBB8',command=add_event,width=110,height=35,border_width=2,font=("arial",15,'bold'),border_color='#1A5319',fg_color="#387478",hover_color='#729762')
add_event_btn.place(x=920,y=615)

delete_event_btn = CTkButton(window, text="Delete",bg_color='#C7DBB8', command=delete_events,width=110,height=35,border_width=2,font=("arial",14,'bold'),border_color='#1A5319',fg_color="#387478",hover_color='#729762')
delete_event_btn.place(x=1039,y=615)

# Listbox to display events for selected date
event_listbox_label =  CTkLabel(window, text="Appointment Today:",bg_color='#C7DBB8',font=('bold',16))
event_listbox_label.place(x=1176,y=472)
event_listbox = tk.Listbox(window, width=34, height=8,font=('arial',15,'italic'))
event_listbox.place(x=1462,y=620)
event_listbox_label =  CTkLabel(window, text="Appointment Setter:",bg_color='#C7DBB8',font=('bold',20))
event_listbox_label.place(x=924,y=92)
